...
/*
  Plugin Name: Feed by Mail Extension for Newsletter
  Plugin URI: http://www.thenewsletterplugin.com/plugins/newsletter/feed-by-mail-module
  Description: Daily or weekly autogenerated newsletter with latest posts
  Version: 2.2.0
...
class NewsletterFeed {
    ...
    function hook_init() {
        ...
        // Sometime other bad written plugin remove schedules...
        if (!wp_next_scheduled('newsletter_feed')) {

            // Create the daily event at the specified time
            if ($this->options['enabled'] == 1) {
                $hour = (int) $this->options['hour'] - get_option('gmt_offset'); // to gmt
                $day = gmdate("d");
                if (gmdate('G') > $hour) {
                    $day++;
                }
    ...
    function hook_admin_menu() {
        add_submenu_page('newsletter_main_index', 'Feed by Mail', 'Feed by Mail', 'manage_options', 'newsletter-feed/index.php');
    }
    ...
    function create_email($options = null, $last_run = null) {
        ...
        $email['total'] = (int) $wpdb->get_var("select count(*) from " . $wpdb->prefix . "newsletter where status='C' and feed=1");

        $email['type'] = 'feed';
        $email['send_on'] = time();
        $email['status'] = 'sending';
        $email['track'] = $options['track'];
        $email['query'] = "select * from " . $wpdb->prefix . "newsletter where status='C' and feed=1";
