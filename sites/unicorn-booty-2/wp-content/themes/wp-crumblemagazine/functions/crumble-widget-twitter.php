...
function CT_Twitter_Widget() {
		...
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'ct_twitter_widget' , 'description' => __( 'Twitter Widget' , 'crumble' ) );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 350, 'id_base' => 'ct_twitter_widget' );
		
		/* Create the widget. */
		$this->WP_Widget('ct_twitter_widget', __( 'CrumbleMagazine: Twitter Widget' , 'crumble' ) , $widget_ops, $control_ops );
...
if ( !function_exists( 'ct_relative_time' ) ) {
	function ct_relative_time($a) {
		...
		if(is_numeric($d) && $d > 0) {
			...
			//if less then minute
			if($d < $minute) return floor($d) . __(' seconds ago', 'crumble');
			//if less then 2 minutes
			if($d < $minute * 2) return __('about 1 minute ago', 'crumble');
			//if less then hour
			if($d < $hour) return floor($d / $minute) . __(' minutes ago', 'crumble');
			//if less then 2 hours
			if($d < $hour * 2) return __('about 1 hour ago', 'crumble');
			//if less then day
			if($d < $day) return floor($d / $hour) . __(' hours ago', 'crumble');
			//if more then day, but less then 2 days
			if($d > $day && $d < $day * 2) return __('yesterday', 'crumble');
			//if less then year
			if($d < $day * 365) return floor($d / $day) . __(' days ago', 'crumble');
			//else return more than a year
			return __('over a year ago', 'crumble');
		}
	}	
}
	...
	if(!empty($tp_twitter_plugin_tweets)){
		...
		foreach($tp_twitter_plugin_tweets as $tweet){
			...
			print '<span class="tweet_text">'.ct_convert_links($tweet['text']).'</span><span class="tweet_time"><a target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.ct_relative_time($tweet['created_at']).'</a></span></li>';
...
function form( $instance ) {
	...
	<p style=" background: #fff2bf; padding: 7px; border: 1px solid #c0c0c0; font-size: 11px; ">
		<?php _e('Also, be sure to fill the Twitter Settings (menu Appearance -> Theme Options -> Twitter settings)','crumble'); ?>
	</p>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e( 'Twitter Username:' , 'crumble'); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'tweetstoshow' ); ?>"><?php _e( 'Tweets to display' , 'crumble' ); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'cachetime' ); ?>"><?php _e( 'Cache Tweets in every:' , 'crumble' ); ?></label>
