...
-----------------------------------------------------------------------------------
 	...
 	Plugin Name: CrumbleMagazine Recent Comments Widget
 	Plugin URI: http://www.color-theme.com
 	Description: A widget that show recent comments ( Specified by cat-id ).
 	Version: 1.0
 	Author: Zerge
...
class CrumbleMagazine_Recent_Comments_Widget extends WP_Widget {
	...
	function CrumbleMagazine_Recent_Comments_Widget()
	{
		...
		/* Create the widget. */
		$this->WP_Widget('crumblemagazine_recent_comments_widget', __( 'CrumbleMagazine : Recent Comments' , 'crumble' ), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		echo '<ul class="recent-comments-widget">';
			foreach( $recent_comments as $comment ) {
				...
				echo '<strong>' . dp_get_author($comment) .' '.__('said:', 'crumble').' </strong>'; ?>
				...
				<?php echo '<div class="clear"></div></li>'; 
			}
		echo '</ul>'; ?>
