<?php if (get_option('ubooty_blog_style') == 'false') { ?>
	...
	<?php $biglayout = ( (bool) get_post_meta($post->ID,'et_bigpost',true) ) ? true : false;
		...
		<?php $medlayout = ( (bool) get_post_meta($post->ID,'et_medpost',true) ) ? true : false;
		
				$thumb = '';
				$width = $medlayout ? 466 : 222;
				$height = 180;
				$classtext = '';
				$titletext = get_the_title();
	...
	<div class="entry <?php if ($biglayout) echo('big'); elseif ($medlayout) echo('med'); else echo('small');?>">
	<div <?php post_class() ?>>
	<div class="thumbnail">
			...
			<div class="comment-buble">
			    <span class="com"><?php comments_popup_link('0', '1', '%'); ?></span>
			</div>
			<div class="category"><?php the_category(); ?></div>
			<span class="month"><?php the_time('M'); ?><span class="date"><?php the_time('d'); ?></span></span>
		</div> <!-- end .thumbnail -->	
		<h2 class="title"><a href="<?php the_permalink(); ?>"><?php if ($biglayout) truncate_title(40); elseif ($medlayout) truncate_title(85); else truncate_title(85); ?></a></h2>
		<p class="postinfo"><?php _e('posted by','ubooty'); ?> <?php the_author_posts_link(); ?></p>
		<div class="entry-content">
			<div class="bottom-bg">
				<div class="excerpt">
					<?php if ($biglayout) truncate_post(100); elseif ($medlayout) truncate_post(200); else truncate_post(80); ?>
					<div class="textright">
